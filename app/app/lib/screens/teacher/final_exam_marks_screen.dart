...
class _FinalExamMarksScreenState extends State<FinalExamMarksScreen> {
        ...
        for (var studentDoc in studentSnapshot.docs) {
          ...
          for (var subjectDoc in _subjectsForSemester) {
            ...
            DocumentSnapshot iaMarkSnapshot = await FirebaseFirestore.instance
                ...
                .doc(iaMarkDocId)
                .get();
            double? iaFinalMark = (iaMarkSnapshot.exists
                ? (iaMarkSnapshot.data() as Map<String, dynamic>)['calculated_iaFinal'] as num?
                : null)?.toDouble(); 
            ...
            int? examFinalMark = finalMarkSnapshot.exists
                ? (finalMarkSnapshot.data() as Map<String, dynamic>)['examFinal'] as int?
                : null; 
            ...
            final controller = TextEditingController(text: examFinalMark?.toString() ?? '');
          ...
          loadedData.add(StudentFinalMarkModel(
            studentId: studentId,
  ...
  Future<void> _saveFinalMark(StudentFinalMarkModel studentModel, String subjectId) async {
      ...
      if (examMark != null && (examMark < 0 || examMark > maxExamMark)) {
         _showError("Exam mark for ${subjectData.subjectCode} must be between 0 and $maxExamMark.");
         return;
      }
      ...
      try {
         // Recalculate Total locally
          ...
          Map<String, dynamic> dataToSave = {
             ...
             'studentRef': FirebaseFirestore.instance.doc('students/${studentModel.studentId}'),
         ...
         _showError("Error saving for ${subjectData.subjectCode}: ${e.toString()}");
      }
  }
  ...
  Future<void> _exportFinalPdf() async {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(content: Text('Generating Final Marks PDF...'), duration: Duration(seconds: 1)),
    );
     await Future.delayed(const Duration(milliseconds: 50));

    final pdf = pw.Document();
    final batchName = Provider.of<AppState>(context, listen: false).selectedBatchName ?? 'Batch';
    ...
    final List<List<String>> data = _studentFinalMarks.map((studentModel) {
       ...
       for (var subjDoc in _subjectsForSemester) {
          ...
          final totalStr = subjectMarkData?.calculatedTotal?.toStringAsFixed(1) ?? '-';
    ...
    try {
          ...
          build: (pw.Context pdfContext) => [
            ...
            pw.Row(
              ...
              children: [
                pw.Column(
                   ...
                   children: [
                     pw.Text('Final Marks Report - Sem $_selectedSemester', style: pw.TextStyle(fontWeight: pw.FontWeight.bold, fontSize: 16)),
                     pw.Text('Batch: $batchName'),
                   ]
                ),
                pw.Text('Generated on: ${DateTime.now().toString().substring(0,16)}', style: const pw.TextStyle(fontSize: 10)),
              ]
            ),
            ...
            pw.Table.fromTextArray(
              ...
              cellAlignments: {
                  ...
                  for (int i = 3; i < allHeaders.length; i++) i: pw.Alignment.center,
              },
              columnWidths: { 
                 0: const pw.FixedColumnWidth(25), 
                 1: const pw.FlexColumnWidth(2.5), 
                 2: const pw.FixedColumnWidth(65), 
                 for (int i = 3; i < allHeaders.length; i++) i: const pw.FlexColumnWidth(1),
              }
            ),
          ],
        ),
      ...
      // --- Save or Share ---
      final pdfFileName = '${batchName}_Sem${_selectedSemester}_Final_Marks.pdf';
      await Printing.layoutPdf(
       ...
       print("Error generating Final PDF: $e");
        if(mounted) {
           ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(content: Text('Error creating Final PDF: ${e.toString()}'), backgroundColor: Colors.red),
           );
        }
    }
  }
  ...
  Widget build(BuildContext context) {
    final selectedBatchName = Provider.of<AppState>(context).selectedBatchName ?? 'Final Marks';
        ...
        backgroundColor: Theme.of(context).colorScheme.inversePrimary,
         actions: [
          IconButton(
            ...
            onPressed: _isLoading || _studentFinalMarks.isEmpty ? null : _exportFinalPdf,
          ),
           ...
           Expanded(
            child: _isLoading
                ...
                : _subjectsForSemester.isEmpty
                    ? Center(child: Text('No subjects found for Sem $_selectedSemester in this batch.'))
                             ...
                             child: SingleChildScrollView( 
                                ...
                                child: SingleChildScrollView( 
                                  ...
                                  child: DataTable(
                                      columnSpacing: 10,
                                      headingRowHeight: 55,
                                      dataRowMinHeight: 48, dataRowMaxHeight: 52,
                                      border: TableBorder.all(color: Colors.grey.shade300, width: 1),
                                      // --- Dynamic Columns ---
                                      columns: [
                                         const DataColumn(label: Text(' Name', style: TextStyle(fontWeight: FontWeight.bold))),
                                         const DataColumn(label: Text(' USN', style: TextStyle(fontWeight: FontWeight.bold))),
                                         // Generate columns for each subject
                                         ..._subjectsForSemester.expand((subjDoc) {
                                            final code = subjDoc['subjectCode'] ?? 'N/A';
                                            return [
                                              DataColumn(label: Text(' ${code}\n (IA)', textAlign: TextAlign.center, style: TextStyle(fontWeight: FontWeight.bold, fontSize: 11))),
                                              DataColumn(label: Text(' ${code}\n (Exam)', textAlign: TextAlign.center, style: TextStyle(fontWeight: FontWeight.bold, fontSize: 11))),
                                              DataColumn(label: Text(' ${code}\n (Total)', textAlign: TextAlign.center, style: TextStyle(fontWeight: FontWeight.bold, fontSize: 11))),
                                            ];
                                         }).toList(),
                                      ],
                                      // --- Dynamic Rows ---
                                      rows: _studentFinalMarks.map((studentModel) {
                                         return DataRow(cells: [
                                            DataCell(SizedBox(width: 130, child: Text(studentModel.studentName, overflow: TextOverflow.ellipsis))),
                                            DataCell(SizedBox(width: 90, child: Text(studentModel.studentUsn))),
                                            ...
                                            ..._subjectsForSemester.expand((subjDoc) {
                                               ...
                                               return [
                                                  // IA Final (Read Only)
                                                  DataCell(Center(child: Text(subjectMarkData?.iaFinal?.toStringAsFixed(1) ?? '-', style: const TextStyle(fontWeight: FontWeight.w500)))),
                                                  // Exam Final (Input)
                                                  DataCell(_buildExamInput(studentModel, subjectId)),
                                                  // Calculated Total (Read Only)
                                                  DataCell(Center(child: Text(subjectMarkData?.calculatedTotal?.toStringAsFixed(1) ?? '-', style: const TextStyle(fontWeight: FontWeight.bold)))),
                                               ];
                                            }).toList(),
                                         ]);
                                      }).toList(),
