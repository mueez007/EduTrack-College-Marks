...
class _MarksEntryScreenState extends State<MarksEntryScreen> {
  ...
  Map<String, double> _classAverages = {
    'ia_1': 0.0, 'ia_2': 0.0, 'ia_3': 0.0, 
    'projectOrAssignment': 0.0, 'iaFinal': 0.0
  };
  // ----------------------------------------
  ...
  void _calculateClassAverages(List<StudentMarkModel> marks) {
    ...
    for (var mark in marks) {
      ...
      if (mark.name.isNotEmpty) {
          ...
          if (mark.projectOrAssignment != null) sumProjAssign += mark.projectOrAssignment!;
          if (mark.calculatedIaFinal != null) sumIaFinal += mark.calculatedIaFinal!;
      }
    }
  ...
  Future<void> _loadStudentMarks() async {
    ...
    try {
      ...
      for (var studentDoc in studentSnapshot.docs) {
        ...
        Map<String, dynamic>? markData = markSnapshot.exists ? markSnapshot.data() as Map<String, dynamic> : null;
    ...
    int? projAssign = markData?['projectOrAssignment'] as int?;
        ...
        final controllers = {
          ...
          'projectOrAssignment': TextEditingController(text: projAssign?.toString() ?? ''),
        };
  ...
  Future<void> _saveMarks(StudentMarkModel studentMark) async {
    ...
    int? projAssign = int.tryParse(studentMark.controllers['projectOrAssignment']!.text);
    
    // Quick validation check before saving
     if ((ia1 != null && (ia1 < 0 || ia1 > (widget.subjectData['baseInternalMax'] ?? 40))) ||
         (ia2 != null && (ia2 < 0 || ia2 > (widget.subjectData['baseInternalMax'] ?? 40))) ||
         (ia3 != null && (ia3 < 0 || ia3 > (widget.subjectData['baseInternalMax'] ?? 40))) ||
         (projAssign != null && (projAssign < 0 || projAssign > (widget.subjectData['maxProject'] ?? 25)))) {
           ScaffoldMessenger.of(context).showSnackBar(
             const SnackBar(content: Text('Error: Mark is outside the allowed range.'), backgroundColor: Colors.red),
           );
           return;
         }
    ...
    try {
      ...
      // Prepare data to save
       Map<String, dynamic> dataToSave = {
        ...
        'ia_3': ia3,
        'projectOrAssignment': projAssign,
        'calculated_iaFinal': calculatedFinal, // Store the locally calculated result
        'batchYear': _selectedBatchId,
        'semester': widget.subjectData['semester'],
        'studentRef': FirebaseFirestore.instance.doc('students/${studentMark.studentId}'),
      ...
      if (mounted) {
        setState(() {
           final index = _studentMarks.indexWhere((sm) => sm.studentId == studentMark.studentId);
           if (index != -1) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
           SnackBar(content: Text('${studentMark.name} marks saved.'), duration: Duration(seconds: 1)),
         );
      }
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Error saving marks for ${studentMark.name}: ${e.toString()}'), backgroundColor: Colors.red),
        );
      }
    }
  }
  ...
  Widget _buildMarkInput(StudentMarkModel studentMark, String fieldKey, int maxMark) {
     String label;
    TextInputAction action;
    FocusNode currentFocusNode = _focusNodes[studentMark.studentId + '_' + fieldKey]!;
    FocusNode? nextFocusNode;
     ...
     switch (fieldKey) {
      ...
      case 'ia_3':
         label = 'IA 3';
         action = TextInputAction.next;
         nextFocusNode = _focusNodes[studentMark.studentId + '_projectOrAssignment'];
         break;
      case 'projectOrAssignment':
        label = widget.subjectData['iaCalculationRule'] == 'SEM_5_6_SCHEMA' ? 'Proj' : 'Asgn';
        ...
        onChanged: (value) {
          ...
          setState(() {
             double previewFinal = _markCalculator.calculateIaFinalLocal(
                ...
                projectOrAssignment: int.tryParse(studentMark.controllers['projectOrAssignment']!.text),
                subjectData: widget.subjectData,
             );
             final index = _studentMarks.indexWhere((sm) => sm.studentId == studentMark.studentId);
             if (index != -1) {
  ...
  Future<void> _exportPdf() async {
    ...
    final batchName = Provider.of<AppState>(context, listen: false).selectedBatchName ?? 'Unknown Batch';
    ...
    try {
      pdf.addPage(
        pw.MultiPage(
          ...
          header: (pw.Context pdfContext) => pw.Container(
              ...
              child: pw.Text('Page ${pdfContext.pageNumber} of ${pdfContext.pagesCount}', style: pw.Theme.of(pdfContext).defaultTextStyle.copyWith(color: PdfColors.grey))
          ),
          build: (pw.Context pdfContext) => [
            ...
            pw.Row(
              ...
              children: [
                pw.Column(
                   ...
                   children: [
                     pw.Text('${widget.subjectCode} - ${widget.subjectName}', style: pw.TextStyle(fontWeight: pw.FontWeight.bold, fontSize: 16)),
                     pw.Text('Batch: $batchName | Semester: ${widget.subjectData['semester']}', style: const pw.TextStyle(fontSize: 12)),
                   ]
                ),
            ...
            pw.Table.fromTextArray(
             ...
             pw.Row(
                ...
                children: [
                   pw.Text('Generated on: ${DateTime.now().toString().substring(0,16)}', style: const pw.TextStyle(fontSize: 9)),
                ]
             )
          ],
        ),
      ...
      // --- Save or Share ---
      final pdfFileName = '${widget.subjectCode}_${batchName}_Sem${widget.subjectData['semester']}_IA_Marks.pdf';
      await Printing.layoutPdf(
       ...
       print("Error generating PDF: $e");
        if(mounted) {
           ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(content: Text('Error creating PDF: ${e.toString()}'), backgroundColor: Colors.red),
           );
        }
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text('${widget.subjectCode} - Marks Entry', overflow: TextOverflow.ellipsis),
                        ...
                        children: [
                          ...
                          Padding(
                            ...
                            child: Row(
                              children: [
                                ...
                                _buildAveragePill('IA 1 Avg', _classAverages['ia_1']!),
                                _buildAveragePill('IA 2 Avg', _classAverages['ia_2']!),
                                _buildAveragePill('IA 3 Avg', _classAverages['ia_3']!),
                                _buildAveragePill('$projAssignLabel Avg', _classAverages['projectOrAssignment']!),
                                _buildAveragePill('IA Final Avg', _classAverages['iaFinal']!),
                              ],
                            ),
                          ),
                          // ------------------------------------
                          ...
                          DataTable(
                              ...
                              columns: [
                                ...
                                DataColumn(label: Text(' IA 1\n (/$maxIA)', textAlign: TextAlign.center, style: TextStyle(fontWeight: FontWeight.bold, fontSize: 12))), 
                                DataColumn(label: Text(' IA 2\n (/$maxIA)', textAlign: TextAlign.center, style: TextStyle(fontWeight: FontWeight.bold, fontSize: 12))),
                                DataColumn(label: Text(' IA 3\n (/$maxIA)', textAlign: TextAlign.center, style: TextStyle(fontWeight: FontWeight.bold, fontSize: 12))),
                                DataColumn(label: Text(' $projAssignLabel\n (/$maxProjAssign)', textAlign: TextAlign.center, style: TextStyle(fontWeight: FontWeight.bold, fontSize: 12))),
                                const DataColumn(label: Text(' IA Final\n (/50)', textAlign: TextAlign.center, style: TextStyle(fontWeight: FontWeight.bold, fontSize: 12))),
                                const DataColumn(label: Text(' Save', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 12))), 
                              ],
                              rows: _studentMarks.map((studentMark) {
                                return DataRow(cells: [
                                  DataCell(
                                    SizedBox( 
                                      width: 150,
                                      child: Text(studentMark.name, overflow: TextOverflow.ellipsis)
                                    )
                                  ),
                                  DataCell(_buildMarkInput(studentMark, 'ia_1', maxIA)),
                                  DataCell(_buildMarkInput(studentMark, 'ia_2', maxIA)),
                                  DataCell(_buildMarkInput(studentMark, 'ia_3', maxIA)),
                                  DataCell(_buildMarkInput(studentMark, 'projectOrAssignment', maxProjAssign)),
                                  DataCell(
                                    Center(
                                      child: Text(
                                        studentMark.calculatedIaFinal?.toStringAsFixed(1) ?? '-', 
                                   ...
                                   DataCell(
                                      IconButton(
                                        icon: const Icon(Icons.save_alt, color: Colors.blueGrey, size: 20),
